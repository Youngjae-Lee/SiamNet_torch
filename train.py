...
# def train(model, loader, optim, loss_func, metrics, device, **kwargs):
#     model.train()
#     drw_param = dict(value=0., iter=0, epoch=0, win_title="Loss",)
#     for epoch in tqdm(range(params.start_epoch+1, params.epoch)):
#         avg_loss = RunningAverage()
#         avg_auc = RunningAverage()
#         progbar = tqdm(loader)
#         loss_list = list()
#         for idx, sample in enumerate(progbar):
#             ref_img_batch = sample['ref'].to(device)
...
#             score_map = model(ref_img_batch, srch_img_batch)
#             loss = loss_func(score_map=score_map, labels=label_batch)
#             if idx % params.display == 0:
#                 display_status(sample['ref'][0], sample['srch'][0], score_map.to('cpu')[0], params.viz)
#                 loss_list.clear()
#             optim.zero_grad()
#             loss.backward()
#             optim.step()
...
#             avg_auc.update(metrics(score_map.detach().cpu().numpy(), label_batch.detach().cpu().numpy()))
#             progbar.set_postfix({"Loss": "%0.4f"%loss_val, "AUC Score": "%0.3f"%avg_auc()})
#             avg_loss.update(loss_val)
#
#             drw_param['loss'] = loss_val
#             drw_param['iter'] = idx
#             drw_param['epoch'] = epoch
#             win = plot_score(params.viz, drw_param, win)
#
#         params.scheduler.step()
#         total_loss += avg_loss()
#         print("Loss%0.4f"% avg_loss())
#         save_dict = {
#             "model": model.state_dict(),
#             "optim": optim.state_dict(),
...
#             "auc": avg_auc.get()
#         }
#         save_model(params.ckpt_path, save_dict)
#     return total_loss/float(params.epoch)
...
def main(args):
    ...
    device = param.device['name']
    num = [int(n) for n in param.device['num'].split(' ')]
